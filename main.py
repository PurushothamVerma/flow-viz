...
def edge_detector(image):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) # converts the image into black and white
    filtered = cv2.medianBlur(gray, ksize=5)       # used to reduce the noise : replaces each pixel's value with the median value of its neighboring pixel
    scharr_x = cv2.Scharr(filtered, cv2.CV_64F, 1, 0) # cal the gradient of the intensity in x dir
    scharr_y = cv2.Scharr(filtered, cv2.CV_64F, 0, 1) # cal the gradient of the intensity in y dir
    edges = cv2.magnitude(scharr_x, scharr_y)         # computes the magnitude of the gradient at each pixel
    threshold_value = 150
    _, binary_edges = cv2.threshold(edges, threshold_value, 255, cv2.THRESH_BINARY)  # applies a binary threshold to the magnitude to remove anything below it
    return binary_edges


def erosion_dilation(binary_edges):
    kernel_size = 3                                                   # kernal size here is 3*3
    kernel = np.ones((kernel_size, kernel_size), np.uint8)            # creates a kernal size 3*3 filled with ones
    eroded_edges = cv2.erode(binary_edges, kernel, iterations=1)      # erosion to remove small moise (sets the kernel center to min pixel value)
    dilated_edges = cv2.dilate(eroded_edges, kernel, iterations=1)    # dilation to enhance the edges (sets the kernel center to man pixel value)
    return dilated_edges

def connecting_components(binary_edges, offset):
    height, width = binary_edges.shape                                                                        # dim of the image
    roi_right = binary_edges[:, width // 2 + offset:]                                                         # want to perform it on the right half on the image only (offset = right to the wing )

    labeled_regions = label(roi_right)                                                                        # label function applies a unique label to each component
    properties = regionprops(labeled_regions)                                                                 #  regionprops function cal properties of the labeled regions such as centroid, area, bbox
    centroids = []

    for prop in properties:                                                                                   # loop iterates over the list of properties and for each connected components, extracts the centroid's row and col cor-ordinates using prop.centroid
...
def dbscan(centroids):
    if len(centroids) > 0:
        ...
        min_samples = 1
        dbscan = DBSCAN(eps=eps, min_samples=min_samples).fit(centroids)                   # fiting DBSCAN model on centroids data
        cluster_labels = dbscan.labels_                                                    # that it belongs to which cluster (maybe I dont need this)
        mean_centroid = np.mean(centroids, axis=0)                                         # cal the mean of the centroids row wise(do i need to do dbscan? i can get this without it)
        average_distance = np.mean(np.linalg.norm(centroids - mean_centroid, axis=1))      # cal the eucledian distance between the meaan centroid and all the other centroids
...
def create_video():
    ...
    os.makedirs(output_folder, exist_ok=True)                                                               # creates output folder if it dosen't exist

    output_filename = "50%_Full_Clap_Vortex_Tracking_Test_3.mp4"
    output_path = os.path.join(output_folder, output_filename)                                              # complete path for the output
    ...
    frame_width = 1920
    frame_height = 1080
    out = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*'mp4v'), 10, (frame_width, frame_height))    # creates the output video (10 fps, should change it to 24 fps)
...
video_path = "/content/50%_Full_Clap.mp4"
